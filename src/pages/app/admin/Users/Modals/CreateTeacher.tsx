...
function CreateTeacherModal({ modalId }: ModalComponentProps) {
  ...
  const submit = async () => {
    try {
      await postData("admin/create-teacher", values);
    } finally {
      resetForm();
  ...
  return (
    <Modal modalId={modalId} title={"Create Teacher"}>
      <ModalHeader modalId={modalId}>
        <Typography variant="subheading">Create Teacher </Typography>
      </ModalHeader>
