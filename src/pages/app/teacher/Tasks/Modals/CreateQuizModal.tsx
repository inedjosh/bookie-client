...
function CreateQuizModal({ modalId }: ModalComponentProps) {
  ...
  const {
    ...
    onSubmit: async () => {
      try {
        await postData(`quiz`, { ...values, course: cohort?.course._id });
      } finally {
        resetForm();
  ...
  return (
    <Modal modalId={modalId}>
      <ModalHeader modalId={modalId}>
        <Typography variant="subheading">Create Quiz</Typography>
      </ModalHeader>
          ...
          {values.quiz.map((q, i) => (
            <div key={i} className=" py-10 border-b">
              ...
              <div className="flex  flex-wrap">
                {q.options.map((option, j) => (
                  <Input
                    ...
                    name={`option_${j}`}
                    handleBlur={handleBlur}
                    // error={errors.quiz?.[i]?.options?.[j]}
                    onChange={(e) => updateQuestion(e, i, j)}
                    label={`Option ${j + 1}`}
                    //   touched={touched.quiz?.[i]?.options?.[j]}
              ...
              <Input
                ...
                name="correctAnswer"
                handleBlur={handleBlur}
                //   error={errors.quiz?.[i]?.correctAnswer}
                onChange={(e) => updateQuestion(e, i)}
                label="Correct Answer"
                touched={touched.quiz?.[i]?.correctAnswer}
